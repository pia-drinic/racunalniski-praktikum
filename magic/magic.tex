...
\begin{definicija}
   {\color{purple}\emph{Magični kvadrat}} reda $n$ je nabor $n^2$ različnih števil,
...
primer Euler, glej Euler \cite{euler}. % ključ: euler
...
\begin{proof}
   ...
   števil (glej ?? \ref{eq:numbers}, \ref{eq:numbers} na strani ??\pageref{}) enaka
...
Preprost račun pokaže, da je konstanti ?? \eqref{label} analogna konstanta
...
Števila različnih normalnih magičnih kvadratov se nahajajo v tabeli ??\ref{}.
% sklic: table:stevila
...
Avtorja navedenega približka sta Pinn in Wieczerkowski (glej Pinn \cite{pinn}), ki
